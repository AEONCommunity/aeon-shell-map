...
function updateNetworkNodeList() {
  return new Promise((resolve, reject) => {
    if (fs.existsSync(config.networkNodeListJSONurl)) {
      let response = fs.readFileSync(config.networkNodeListJSONurl)
      let responseAsJson = JSON.parse(response)
      globals.networkNodeList = responseAsJson
      log('info', logSystem, 'Updated network node list. Total: %s', [globals.networkNodeList.nodes.length])
